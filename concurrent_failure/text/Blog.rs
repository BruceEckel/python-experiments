...
If a task throws an exception within a TaskGroup, that exception “becomes/is contained in” an ExceptionGroup, which is a type of Exception. And notice that all the errors produced by all the tasks in the group are collected into a list within that single ExceptionGroup, after the first argument that describes the exception.
...
Picking apart exception groups made the feature less-palatable, so they added a way to capture specific types within an exception group. To use this, you say except* (a keyword added to support exception groups) instead of just except:
...
Each except* clause picks out all exceptions of that particular type, and puts those inside another ExceptionGroup. With except* AttributeError, you can see how both instances are combined into the resulting ExceptionGroup.
